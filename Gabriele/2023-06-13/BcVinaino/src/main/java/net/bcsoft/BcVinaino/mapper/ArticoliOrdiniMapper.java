...
public interface ArticoliOrdiniMapper {
    @Insert({"INSERT INTO articoli_ordine (id_ordine, id_menu, quantita) VALUES (#{id_ordine}, #{id_menu}, #{qta}) "})
    ...
    @Update({"UPDATE articoli_ordine SET id_ordine = #{idOrdine}, id_menu = #{idMenu}, quantita = #{qta} WHERE id_articoli_ordine = #{idArticoliOrdine}"})
    public int update(ArticoliOrdini articoliOrdini);

    @Select({
            "SELECT id_articoli_ordine, id_menu, id_ordine, quantita FROM articoli_ordine",
    ...
    @Results(id = "articoli_ordine", value = {
            ...
            @Result(column = "quantita", property = "qta")
    })
    public List<ArticoliOrdini> selectAll();

    @Select({
            "SELECT id_articoli_ordine, id_menu, id_ordine, quantita FROM articoli_ordine WHERE id_articoli_ordine = #{articoliOrdineId, jdbcType=NUMERIC}" //con il cancelletto prende il parametro alla riga 26 e lo butta dentro
